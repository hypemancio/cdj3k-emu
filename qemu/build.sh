...
#
# [intel-port] Some of those tools are optional at configure time (elf2dmp
# only exists when libcurl is found - true on Homebrew setups, not on
# MacPorts). meson install only wants the targets this configure actually
# generated, so probe build.ninja and build exactly that subset.
WANTED_TARGETS="qemu-system-aarch64 \
...
for t in ${WANTED_TARGETS}; do
    if ninja -C "${BUILD_DIR}" -t query "$t" >/dev/null 2>&1; then
        BUILD_TARGETS="${BUILD_TARGETS} $t"
    else
        echo "    [skip] $t (not generated by this configure)"
    fi
done
